...
CageLab.Viewer = function (canvas_width, canvas_height){
	...
	this.materials.cage_material = new THREE.MeshBasicMaterial({
	...
	interaction_toolbar_event: function(active_now){
		...
		switch(active_now) {
			...
			case 'move_handles':
				...
				if(this.cage.active_handles.length > 0){
	...
	load_mesh: function(url, name) {
		...
		if(this.is_mesh) {
			...
			this.loaders.file_loader.load(url, function (data) {
				...
				var geometry = CageLab.app.create_geometry( CageLab.app.cage.vertices, CageLab.app.cage.faces );
				
				CageLab.app.cage.wireframe = new THREE.Mesh( geometry, CageLab.app.viewer.materials.cage_material);
...
*/
	...
	*/
	updateCage: function(){
		...
		this.cage.wireframe.geometry.attributes.position.setArray( geometry );
		this.cage.wireframe.geometry.attributes.position.needsUpdate = true;
	...
	updateModel: function(){
		...
		var new_mesh_vertices;
		
		if (this.barycentric_mode === this.barycentric_coords.MVC) new_mesh_vertices = this.backend.deformMVC( cage_vertices );
		else new_mesh_vertices = this.backend.deformGreen( cage_vertices );
	...
	parse_geometry: function(data){
		...
		if(this.is_mesh){
			...
			this.mesh.vertices = this.mesh.vertices.map(function(x)  { return x * 200 } );
			...
			this.cage.vertices = this.cage.vertices.map(function(x)  { return x * 200 } );
	...
	animate: function() {
		...
		if( this.controls.needs_update && this.controls.moveHandles ){	
			this.updateCage();
			if(this.barycentric_mode !== this.barycentric_coords.NONE)	this.updateModel();
		}
